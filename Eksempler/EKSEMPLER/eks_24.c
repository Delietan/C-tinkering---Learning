...
int main() {
    ...
    printf("\nSkriv inn datoer, og faa vite hvilket dagnummer det er i aaret.");
    printf("\nAvslutt med en ulovlig dato, f.eks. 30.februar ");
    printf("et eller annet aar.\n\n");

    do  {
        dag    = lesTall("Dag  ", 1, 31);
        maaned = lesTall("Maaned", 1, 12);
        aar    = lesTall("Aar   ", 1600, 2100);

        if ((dagNr = dagNummer(dag, maaned, aar)) != 0) // Skriver dagummeret:
           printf("Datoens nummer i det aaret:  %3i\n\n", dagNr);
        else
