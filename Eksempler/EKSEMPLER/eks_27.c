...
void endreAntallet(const enum Endring ms)  {
   ...
   if (gAntallVarer > 0)  {                //  Noen vare(r) er registrert:
      ...
      printf("\n\tVaren har %s antallet med %i til %i stk.\n",
             (ms == Selge) ? "redusert" : "okt",
             endreAntall, gVarer[endreNr-1].antall);
   } else
      printf("\n\tVarelageret er TOMT - umulig aa endre ant. av en vare.\n");
}
...
void fjernVare()  {
   ...
   if (gAntallVarer > 0)  {                   //  Noen vare(r) er registrert:
      ...
      if (fjerneNr != 0)  {                   //  Relevant varenummer:
         ...
         printf("\n\tVare nr.%i er naa fjernet.\n", fjerneNr);
      } else
         printf("\n\tOK, du angrer - ingen vare er fjernet.\n");
  } else
     printf("\n\tVarelageret er TOMT, derfor umulig aa fjerne noen vare.\n");
}
...
void nyVare()  {
  if (gAntallVarer < MAXVARER)  {    //  Fortsatt plass/mulig:
     ...
     printf("\n\tAlt om den nye varen er naa ferdig registrert.\n");
...
void oversikt()  {
  ...
  if (gAntallVarer > 0)  {           //  Vare(r) er registrert:
     printf("\n\tVARELAGERET INNEHOLDER FOLGENDE VARER:\n");
     printf("\n\t Nr  Antall      Pris  Navn\n");
     ...
     printf("\n\tVarelageret er TOMT, derfor umulig aa lage en oversikt.\n");
}
...
void skrivMeny()  {
   ...
   printf("\tM = Mottatt/faatt 'x' stk. av en vare\n");
