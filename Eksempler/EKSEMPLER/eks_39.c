...
void skrivMeny()  {
   ...
   printf("\tV - ny Vin\n");
   printf("\tE <nr>  <(nytt) aar>  <ant.flasker inn(+)/ut(-)>\n\t");
   printf("      - Endre vin ( (ny) aargang og/eller antall ");
...
void skrivTilFil()  {
  ...
  if (utfil  &&  utfil2) {                       //  Begge filene kan åpnes:
    ...
    printf("Klarte ikke aa aapne minst en av filene aa skrive til!\n\n");
...
void vinEndre(struct Vin* v)  {
  ...
  if (aar >= 1900 && aar <= 2027) {              //  Årgang er i OK intervall:
     ...
     if (nr != -1) {                              //  Årgangen finnes:
        if (ant > 0 || -ant <= v->antall[nr]) {   //  Får nye eller nok igjen:
           ...
           printf("\n\tAntall paa lager av aargang %i:  %i\n\n", // Skriver nytt
                 aar, v->antall[nr]);                          //   antall.
        } else                                 //  Prøver å fjerner for mange:
          printf("\n\tHar kun %i stk. paa lager!\n\n", v->antall[nr]);
       ...
       if (ant >= 0) {                         //  Positivt antall flasker:
          if (v->antallAarganger < MAXAAR) {   //  Plass til flere årganger:
            ...
            printf("\n\tFullt med %i aarganger!\n\n", MAXAAR);
       } else                         //  Umulig med negativt nytt antall:
         printf("\n\tUmulig med negativt antall av en ny aargang!\n\n");
     }

  } else                                 //  Årgang utenfor intervall:
    printf("\n\tAarstall utenfor aktuelt intervall: 1900-2027.\n\n");
}
