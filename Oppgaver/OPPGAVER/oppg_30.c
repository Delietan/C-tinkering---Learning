...
void finnLaaner()  {
  ...
  lesTekst("Trallelaaners navn", navn);   //  Leser aktuell låners navn.
...
void innlevering()  {
  ...
  if (gAntallUtlaant > 0)  {              //  Minst EN tralle er utlånt:
     ...
     if (gTraller[nr-1].utlaant)  {       //  Trallen er utlånt:
       ...
       printf("\n\tDenne trallen er IKKE utlaant!\n");
  } else                                  //  Ingen tralle er utlånt:
     printf("\n\tUmulig aa innlevere, da ingen utlaant!\n");
}
...
void ledige()  {
  if (gAntallUtlaant < MAXTRALLER)  {         //  Traller er lånt ut:
     ...
     printf("\n\tINGEN traller er for oyeblikket ledige!\n");
}
...
int lesTall(const char tekst[], const int min, const int max)  {
   int tall;
   do  {
     printf("\t%s (%i til %i):  ", tekst, min, max);
...
void skrivMeny()  {
   ...
   printf("\tU - Utlaan av tralle\n");
   printf("\tI - Innlevering av tralle\n");
   printf("\tF - Finn trallene(e) laant av en gitt person\n");
...
void utlaan()  {
  ...
  if (gAntallUtlaant < MAXTRALLER)  {   //  Flere traller er til låns:
     ...
     if (!gTraller[nr-1].utlaant)  {    //  Trallen er ledig:
       ...
       printf("\n\tDette trallenummeret er allerede utlaant!\n");
  } else                               //  Alle trallene er lånt ut:
     printf("\n\tUmulig aa laane flere, da ALLE trallene er utlaant!\n");
}
